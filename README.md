...
To get this running on your own computer, you will need WAMP (or equivalent based on your operating system) and NPM (provided when
installing node.js). Make sure WAMP is running

First, unzip all the files into a folder where your server's public HTML content can be displayed. This can be a subfolder if you
have other projects, or from the root. In src/App.js, set serverURL and imageURL to WAMP's relative path of your project (it's
currently set to be a subfolder called settlerswarlords; you can change it to whatever, this only affects matters during development).

Next, import the database into MySQL. The file is called databaseBackup.sql

Next, navigate to the project's root folder using Command Prompt, and run 'npm install'. This will set up everything needed for the
client code.

Once done, you can run 'npm start' to start the client code. If WAMP is running and your serverURL & imageURL variables are set up,
you will be ready to go!

I have a lot of plans for this game, but it is all subject to change.

## Map construction

The world map will consist of an overview map, made of land plots that will extend indefinitely in all directions. Each land plot will consist
of one of many possible land types. The selection of land types will be decided upon by a cluster-based algorithm (the same one I now use in
DanIdle). The layout of the map may be altered even further by the existence of land-altering civilizations/creatures (not human). For example,
Goblins will live only where lava is exposed on the surface. Other civilizations/creatures will only exist in certain
biomes; for example, Elves will exist only in forests.

Players will have to have ownership of a given land before they can build on it. Many types of non-player civilizations / creatures can have
existing ownership of a land, which the player will need to defeat before taking it over.

Players will have a 'king' unit, which is where the player is located. They are free to travel anywhere within their kingdom, with a time
delay in travelling (for now that will be 5 minutes). Players will only be able to view and directly edit the worldmap tile they reside in.
Players may assign map-makers to create maps of other areas, for easier viewing.

Players will control other lands they claim by way of messengers, both to send orders (such as build structures) and receive reports (such
as inventory supplies). Reports can be set up to be repeated over a set time (send an army report every 24 hours). Messengers may also be
sent from the other lands in the event of major news (such as being under attack). All communication of this type will suffer from travel
lag.

Players will only be shown the portion of the world map that they are aware of. Players will need to send explorers to new lands to determine
what is there, which should be done before sending armies to conquer it. Certain non-player civilizations are not friendly to visitors, and
so explorers sent by players may end up dead. The player will be unaware of this until their explorer has failed to return. Players may choose
to capture or kill any explorers from other players (if said explorer is unable to avoid detection).

If a player has ownership of a land, they can give it a name. This will remove the need to show world map coordinates to players. Players can
also place down sign posts (by spending some wood), which names a given land even after they lose ownership of that land. The new owner of that
land can give it a new name by tearing down the signs. Nearby land plots, if not named, will receive names relative to that land plot (so long
as a player is aware of the named land). So a land plot may be named "2 east of Sam's Hollow". Lands named like this will only be relative to
the closest named land. New players will have a name assigned to the land they start on; we will probably call this 'home'.

## Local map

Each world map plot on the map will contain a set of map squares, as a 'local' map. The current number of squares per plot is 64 in an 8x8 grid.
Players can build structures on these lands, to allow new actions to be performed.

The tiles of this land will be a randomized mix of tile types, ranging from grass to exposed ores. The types available will be determined by the
type of world tile it is; for example, a desert tile will consist of mostly sand, but have patches of grassland and rocks, and a small chance to
show trees. Not all land tiles will be usable. Some will be rock out-croppings, rivers, lakes or other perils. Some of these may be worked into
useful land by lots of work, though.

Each of these local maps will have its own inventory, which the player will have to manage between each of them to keep everything running
smoothly. That being said, the map tiles should be allowed to manage themselves to some degree. The player can set up routine resource routes
between map tiles (for example, iron ores leading to a refinery location, iron ingots leading to other places). If a land plot does not have
sufficient resources to continue working, they will report this to the player, while providing possible sources to grab those goods from. It
will be up to the player to approve / deny this, or change the input rates. If a land plot runs out of resources for a certain product, they
will simply stop making that product. If the land plot runs out of food, it may become deserted.

Land plots will have a road quality level (for the whole plot), which determines how hard it is for convoys to travel through. The player
can send workers and resources to improve the road quality score, thus allowing resources to flow faster through those lands. Road quality
levels will also affect how fast troops can move - both friendly and enemy, meaning players can choose to destroy roads while on retreat.
Road quality will degrade over time & heavy use.

Land plots will be given a stability/safety score. If this is high, immigrants are more likely to settle there verses a low score. Fortifying
a given land plot can increase this score, while attacks on this plot or nearby plots (whether in the same kingdom or not) can reduce the
score. Dangerous non-player civilizations nearby can reduce this score as well, until they are eliminated.
...
Loyalists - These will consist of crew, converted to fight. They will not have much training, and may choose to not follow orders not relating
to their home (such as attack distant lands). They will be able to receive training to improve their effectiveness in combat, and
can be given suitable gear to do even better. They will remain loyal as long as they have shelter and food.

Warriors - These will consist of better-trained loyalists, but will require regular payments to remain as warriors. These may be ordered to
attack distant lands

Commanders - These will consist of warriors who lead troop groups (armies) into battles. Armies will need at least one commander, but may use
...
Stick to a small list of animal types, to each fill specific roles. No need to get fancy (yet. we can expand this later)
...
-   Elephants - much larger than horses. Harder still to capture & tame, but do well in battle. Travels in herds of 50 or more. Not common to find
-   Bear - Large and dangerous, especially for new camps. Only travels alone or with cubs (making them more dangerous)
-   Coyotes - good desert animals, covers a lot of ground at night (wolves can fill this role, too)
-   Goats
...
-   Elementals - creatures made of specific elements. Many cannot be harmed by normal weapons

### Other dangerous creatures

(we may still keep these)
...
-   Armor-plated cow-beasts (needs name)
...
Players will need to produce tools, along with resources. These tools will be consumed along with the input resources, when producing items.
Better efficient tools can be swapped out when they become available, improving speed and resource cost (and sometimes input goods cost).
...
The fortification level will be independent from the productivity level. This means that fortification level can be upgraded, and then the
type of building can be changed out, without losing all the progress in fortification upgrades.
...
-   Orchards - Places used to grow & care for fruit trees. Trees will take a long time to mature, but once they do, will produce lots of food.
