...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Mar 17, 2023 at 09:11 PM
...
CREATE TABLE IF NOT EXISTS `sw_error` (
  ...
  `content` text NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=79 DEFAULT CHARSET=ascii COMMENT='used for error tracking';
...
CREATE TABLE IF NOT EXISTS `sw_map` (
  ...
  `blocks` text CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL COMMENT 'JSON of all running blocks in the map',
...
) ENGINE=MyISAM AUTO_INCREMENT=2079253 DEFAULT CHARSET=latin1 COMMENT='world map';
...
CREATE TABLE IF NOT EXISTS `sw_player` (
  ...
  PRIMARY KEY (`id`),
  KEY `id` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=241 DEFAULT CHARSET=ascii COMMENT='all users';
